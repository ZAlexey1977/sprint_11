...
// свойство _isResident по умолчанию равное true. И свойство приватное _cost - число, равное нулю.
// Создайте конструктор, который принимает свойство isResident и сost в указанном порядке. Напишите метод taxSum,
// который  возвращает т сумму налога с  указанной _cost. Для резидента используется коєффициент 0.05 из массива, для не резидента - 0.21.

class Test_03 {
  private _taxIndex: number[] = [0.05, 0.21];
  private _isResident: boolean = true;
  private _cost: number = 0;
  constructor(isResident: boolean, cost: number) {
    this._isResident = isResident;
    this._cost = cost;
  }
  taxSum(): number {
    if (this._isResident == true) {
      return this._cost * this._taxIndex[0];
    } else {
      return this._cost * this._taxIndex[1];
    }
  }
}
...
// Task 05
// Посмотрим как private свойство ведет себя в наследуемых классах. Снимите комментарий с класса Test_05. Изучите ошибку...
// Ошибка возникает потому что приватное свойство видно только в классе где объявлено.
...
// Task 06
// Давайте изучим как работает protected свойства. Как гласит теория, protected свойство доступно как внутри класса,
...
// Task 08
// Теперь давайте поработаем с конструктором. Создайте класс Test_08, задайте ему private свойство _count - число.
...
// Реализуйте в нем метод test(), который реализует код:
//this.rand()
//return this.n;
